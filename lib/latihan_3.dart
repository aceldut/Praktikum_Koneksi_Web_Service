...
class _MyAppState extends State<MyApp> {
  late Future<List<University>>
      futureUniversities; // Variabel untuk menyimpan hasil dari permintaan API
...
// Fungsi untuk mengambil data universitas dari API
  Future<List<University>> fetchUniversities() async {
    ...
    if (response.statusCode == 200) {
      ...
      return data
          ...
          .toList(); // Mengonversi setiap objek JSON menjadi instance University
    } else {
  ...
  void initState() {
    ...
    futureUniversities =
        fetchUniversities(); // Memuat data universitas saat widget pertama kali dibuat
  }
    ...
    return MaterialApp(
      ...
      home: Scaffold(
        appBar: AppBar(
          ...
          //appbar
          title: const Text('Menampilkan Universitas dan situs'), //judul appbar
        ),
        body: Center(
          //menempatkan pada posisi tengah atau center
          child: FutureBuilder<List<University>>(
            future:
                futureUniversities, //menggunakan fututebuilder untuk list university
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                // Jika masih dalam proses pengambilan data
                return const CircularProgressIndicator(); // Tampilkan indikator loading
                  ...
                  itemBuilder: (context, index) {
                    return Container(
                      ...
                      padding: const EdgeInsets.all(
                          14), //padding didalam containernya
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment
                            .center, //atur kolom untuk tetap berada di tengah
                        children: [
                          Text(snapshot.data![index]
                              .name), //tampilkan nama universitas pada API
                          Text(snapshot.data![index]
                              .website), //tampilkan situs web universitas juga
                        ],
                      ),
                    );
                  },
